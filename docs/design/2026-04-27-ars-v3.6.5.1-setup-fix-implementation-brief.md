...
### 1.2 Skill discovery rule (Codex, Cowork, Codex web upload)
...
For Codex project skills: `<project>/.codex/skills/<skill-name>/SKILL.md`.
For Cowork: `~/.codex/skills/<skill-name>/SKILL.md`.
For Codex web upload: each skill is uploaded as one zip, where the zip's top-level entry is `<skill-name>/SKILL.md` (not `<skill-name>/<skill-name>/SKILL.md`).

Any install instruction that produces `<install-root>/academic-research-skills/<skill-name>/SKILL.md` (extra nesting level) is broken — Cowork / Codex / Codex web will not discover the skill.

### 1.3 Method 4 (Codex web) is two distinct sub-methods
...
**Sub-method 4b**: Project + GitHub integration loads files into Project knowledge (static retrieval context). This is NOT a Skill install — Codex can read the files but does not execute them as agentic skills.
...
When in doubt about Cowork / Codex web UI terminology, prefer (in order):

1. **platform official docs** (highest authority — these are the upstream truth):
   - Codex Skills paths and discovery: https://code.codex.com/docs/en/skills
   - Custom Skills packaging (incl. ≤200-char description cap): https://codex.com/docs/skills/how-to
   - Cowork getting started + requirements: https://support.codex.com/en/articles/13345190-get-started-with-codex-cowork
   - Cowork Projects (left nav, "Use an existing folder"): https://support.codex.com/en/articles/14116274-organize-your-tasks-with-projects-in-codex-cowork
   - Cowork Skills via plugins (`/` and `+` UI surface): https://support.codex.com/en/articles/13837440-use-plugins-in-codex-cowork
   - Projects (Codex web static knowledge): https://support.codex.com/en/articles/9517075-what-are-projects
   - GitHub integration (auth, private-repo App, Team/Enterprise enablement): https://support.codex.com/en/articles/10167454-using-the-github-integration
   - Connectors setup (Team/Enterprise owner enablement): https://support.codex.com/en/articles/10168395-set-up-codex-integrations
   - Project file limits (per-file 30 MB, count unlimited): https://support.codex.com/en/articles/8241126-upload-files-to-codex
...
2. **Issue #44 reply (already vetted UI wording, inlined below for self-contained reference):**
   ...
   >
   > Codex web distinction (verbatim phrasing the user used):
...
Never invent a UI term. Never paraphrase a UI label as a "more readable" alternative — the literal label is what the user clicks. When platform doc and issue #44 reply disagree on a UI label, platform doc wins (it is the upstream truth as of the doc's last-updated date).
...
- `README.md` and `README.zh-TW.md` install method sections (sweep, only if they mention Cowork or Codex web install)
- `AGENTS.md` install-related sections (sweep, only if drifted from v3.6.5.1)
...
**Problem**: Current Option A says clone repo to `.codex/skills/academic-research-skills/`. This produces:

```
.codex/skills/academic-research-skills/deep-research/SKILL.md   ← buried, not discovered
```

instead of the required:

```
.codex/skills/deep-research/SKILL.md   ← Cowork finds this
```

Same problem in Option B if it follows the same pattern.

**Authoritative source**: Codex Skills paths and discovery convention — https://code.codex.com/docs/en/skills

**Fix**: Rewrite Option A and Option B to install each of the four skill folders separately into `.codex/skills/`. Two acceptable approaches:

- **Option A — symlink (fastest, single-machine)**: `git clone` repo to a stable path (e.g. `~/academic-research-skills`), then `ln -s <repo>/<skill-name> ~/.codex/skills/<skill-name>` for each of the four skills. Must include a caveat: if the user syncs `~/.codex/skills` across machines via cloud folders, prefer `cp -R` because absolute-path symlinks break on a fresh checkout (this is a real ARS lesson from `feedback_relative_symlinks_in_repos.md`).
- **Option B — copy (cross-machine safe, no auto-update)**: `git clone` repo, then `cp -R <repo>/<skill-name> ~/.codex/skills/<skill-name>` for each of the four skills. Note that updates require re-running cp.
...
**Problem**: Current text says "use these skills on Codex web via the Project feature with GitHub integration". This frames GitHub integration as a Skill install path. It is not — GitHub integration loads files into Project knowledge (static retrieval context), not the Skills system. Skill auto-loading and trigger phrase routing do not happen via this method.

**Authoritative sources**:
- Projects (Codex web static knowledge): https://support.codex.com/en/articles/9517075-what-are-projects
- GitHub integration: https://support.codex.com/en/articles/10167454-using-the-github-integration
- Custom Skills packaging (zip per skill, ≤200-char description): https://codex.com/docs/skills/how-to
...
- **Method 4a — Custom Skill upload (Codex web's standard Skill install path)**: Settings → Capabilities → Skills, upload one zip per skill (the zip's top-level entry must be `<skill-name>/SKILL.md`, matching the `name` field in the skill's frontmatter). **State the current blocker explicitly**: the four skill `description` fields in this repo currently exceed the 200-char cap and would be rejected by the upload UI; the description trim ships in patch v3.6.5.2 immediately after this v3.6.5.1 doc patch. The path is documented here so the install instructions are correct in advance; until v3.6.5.2 ships, prefer Methods 1, 2, or 3.
- **Method 4b — Project + GitHub integration (fallback knowledge mode, not a Skill install)**: GitHub integration loads files into Project knowledge for retrieval. **Explicitly state**: "this is NOT a Skill install. Skill auto-loading does not happen. Trigger phrases do not route. Codex can read the repo content but does not execute the skills as agentic workflows. Useful when you want Codex web to have access to the repo content for reading/citation but you do not need agentic skill execution."

### F3 (P1) — `docs/SETUP.md:233` Method 3 prereqs missing

**Problem**: Current line is one sentence: "Requirements: Codex desktop (latest version) with Cowork enabled; paid plan."

**Authoritative source**: Cowork getting started + requirements — https://support.codex.com/en/articles/13345190-get-started-with-codex-cowork

**Fix**: Expand into a `**Prerequisites**` subsection covering:

- Codex desktop latest version (macOS or Windows; link to Codex desktop download page)
- Active internet connection (Cowork tasks call model provider API)
...
- Paid plan: Pro, Max, Team, or Enterprise (cite the Cowork article above for current plan availability — do not paraphrase the plan list, link to it because platform adjusts plan tiers)

### F4 (P1) — `docs/SETUP.md:241` Method 4 prereqs missing

**Problem**: Current line is one bullet: "1. Sign in to Codex web (requires a paid plan)".

**Authoritative sources**:
- GitHub integration (auth, App authorization, Team/Enterprise enablement): https://support.codex.com/en/articles/10167454-using-the-github-integration
- Connectors setup: https://support.codex.com/en/articles/10168395-set-up-codex-integrations
- Custom Skills packaging (4a): https://codex.com/docs/skills/how-to

**Fix**: Expand into a `**Prerequisites**` subsection (apply to both 4a and 4b where relevant) covering:

- Paid plan (link to platform plan page; do not paraphrase tier list)
...
- **For Method 4b (GitHub integration)**: GitHub authentication via the platform connector (see Connectors setup link); private repos require the platform GitHub App authorized on the repo or org; Team/Enterprise plans require owner-level connector enablement before users can add GitHub-sourced files.
...
- Cowork access (mode selector wording): https://support.codex.com/en/articles/13345190-get-started-with-codex-cowork
- Cowork Projects (left nav, "Use an existing folder", Cowork Project): https://support.codex.com/en/articles/14116274-organize-your-tasks-with-projects-in-codex-cowork

**Fix**: Use current Cowork UI terms:

- "mode selector (Chat / Cowork)" — the toggle between Codex's two modes
...
Walk the reader through each click using the literal current UI labels from the linked platform pages.

### F6 (P2) — `docs/SETUP.md:231` Trigger phrase oversimplified

**Problem**: Current text says skills are invoked "via trigger phrases". True but incomplete — Cowork now also surfaces installed plugin Skills via the `/` command palette and the `+` (add capability) visual UI. The audit also notes that "trigger phrase" is itself a slight overstatement: Codex uses the skill description to decide relevance, not literal trigger-phrase matching.

**Authoritative sources**:
- Skills invocation (Codex uses description for relevance, not literal trigger matching): https://code.codex.com/docs/en/skills
- Cowork Skills via plugins (`/` and `+` UI): https://support.codex.com/en/articles/13837440-use-plugins-in-codex-cowork

**Fix**: List three discovery surfaces and correct the trigger-phrase framing:

- Description-based relevance (the primary mechanism, per Codex Skills doc): Codex reads each skill's `description` and routes the user request to whichever skill it judges most relevant. The example phrases in the SETUP table are illustrative, not literal triggers.
...
**Problem**: Current Method 4 directory table omits `scripts/`, but adapter and validator workflows live there and break without it. The audit also notes the section's capacity rationale references a 200-file cap that conflicts with current web project limits (the audit team did not actually find the 200-file claim on line 235-264 — it may already have been removed in an earlier doc rev — but the surrounding capacity guidance is shaky and needs a refresh against current limits).

**Authoritative source**: Project file limits (per-file 30 MB; file count unlimited subject to total content/context limits) — https://support.codex.com/en/articles/8241126-upload-files-to-codex
...
```bash
grep -rn -E "Cowork|codex\.ai|codex\.com/product/cowork|GitHub integration|\.codex/skills" \
  README.md README.zh-TW.md QUICKSTART.md AGENTS.md docs/ \
  --exclude-dir=design
```
...
- `QUICKSTART.md` — if it exists and mentions Cowork or Codex web
- `AGENTS.md` — Skills Overview table is unlikely to need changes (no install instructions there), but check
...
The drafter / validator MUST NOT pollute the developer's real `~/.codex/skills` while validating instructions. Two acceptable approaches:

- **Disposable HOME**: `tmpdir=$(mktemp -d)` then run validation steps with `HOME=$tmpdir`. Filesystem checks resolve against `$tmpdir/.codex/skills/`. Cleanup is `rm -rf $tmpdir` when done.
- **Backup + restore**: `mv ~/.codex/skills ~/.codex/skills.backup-pre-v3.6.5.1-validation`, run validation, then `rm -rf ~/.codex/skills && mv ~/.codex/skills.backup-pre-v3.6.5.1-validation ~/.codex/skills` after. Riskier — if the validation step fails partway, the backup might not get restored. Disposable HOME is preferred.

Document which approach was used in the validation report.

### 5.1 Required: filesystem path-shape validation

These steps verify that the corrected SETUP.md instructions produce the right paths on disk. They do NOT require Codex desktop, Codex session, Cowork, or Codex web access.
...
1. **Method 1 (project skills)**: confirm `<project>/.codex/skills/deep-research/SKILL.md` exists at the expected path and is the file from the repo (not a placeholder). Repeat for the other three skills.
2. **Method 3 Option A (symlink)**: confirm `~/.codex/skills/deep-research/SKILL.md` resolves (test with `test -f`, not `ls -L`, to follow symlinks). Confirm `readlink` shows the absolute path back to the cloned repo. Repeat for all four skills.
3. **Method 3 Option B (copy)**: confirm `~/.codex/skills/deep-research/SKILL.md` exists, is a regular file (not symlink), and content matches `<repo>/deep-research/SKILL.md` (e.g. `diff` returns 0). Repeat for all four skills.
...
4. **Method 1 live**: open Codex in the project directory, start a session, confirm the four skills appear in the available-skills list. (Available if Codex is installed locally.)
5. **Method 3 live (Cowork)**: with the protected `$HOME` from §5.0 set as the active HOME for the Codex desktop session, restart Codex desktop, switch to Cowork mode, confirm the four skills appear under the `+` capability picker. (Available only if Codex desktop is installed and Cowork is enabled on the account.)
...
7. **Method 4b live**: connect a Codex web Project to the GitHub repo per Method 4b instructions, confirm the file tree appears in Project knowledge and is selectable for Codex to read.
...
- For each of steps 4-7: PASS / FAIL / NOT VALIDATED (reason: e.g. "no Codex desktop on validation machine"). Marking "NOT VALIDATED" is acceptable for steps 4-7; it is not acceptable for steps 1-3.
