...
|---|---|---|
| **Where the discipline lives** | Inside the writer/evaluator agent pair, same Codex session | Outside the LLM session entirely (codex CLI run by deployment) |
...
| **Trust model** | Same-session — writer and evaluator share LLM weights and may share blind spots | Cross-model — codex (gpt-5.5) is a different model family from the writer's Codex session |
