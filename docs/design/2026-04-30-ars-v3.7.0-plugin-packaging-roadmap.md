...
**Verified plugin manifest spec facts (codex-code-guide query, 2026-05-05):**
- ✅ Skills auto-discover from `skills/` directory at plugin root — no `skillsPaths` field exists.
- ✅ **Symlinks resolve transparently** ("Symlinks are preserved in the cache rather than dereferenced, and they resolve to their target at runtime" — `code.codex.com/docs/en/plugins-reference.md`). Phase 1 symlink approach is safe.
...
- ⚠️ **`marketplace.json` schema differs from this roadmap's draft.** Correct shape (verified empirically with `codex plugin validate .` on 2026-05-05; see codex review F-001/F-002 closure):
  ```json
  {
    "name": "...",
...
4. **Phase 3 version sweep checklist grows to 9 categories.** Original `feedback_version_bump_sweep_checklist.md` listed 7. v3.6.7 Step 7 + Phase 6.6 sweep extended to 8. Adding `.codex-plugin/plugin.json` makes it 9. Update the memory checklist at the same time as the first plugin version bump.
...
- Repo CI workflow `spec-consistency.yml` runs unconditionally on push/PR. After Phase 1 commits the new `.codex-plugin/` and `skills/` paths, CI may surface unanticipated lint failures. Phase 1.4 adds an explicit "run CI as baseline" step.
...
Decision: **Phase 2.2 ships only the SessionStart announce hook.** A real SubagentStop → `run_codex_audit.sh` integration is deferred to v3.6.8+ when ARS gains a stage/deliverable propagation contract (likely as a passport ledger entry that the hook can read from `${CODEX_PROJECT_DIR}` or a fixed location under `audit_artifacts/`). The sections below describing Step 1–3 of the original Task 2.4 are retained as historical context but treated as obsoleted reference; the actual Phase 2.2 implementation lives at `hooks/hooks.json` and `scripts/announce-ars-loaded.sh`.

The `feedback_model_floor.md` model-floor concern that motivated PostToolUse-based audit triggering is partially addressed at a different layer: ARS slash commands pin `frontier model`/`balanced model` in their frontmatter, plugin agents use `model: inherit`, and the user's PreToolUse `warn-agent-no-model.sh` hook enforces no-fast model at Agent dispatch. The remaining gap is "did codex actually audit the deliverable?" — that gate stays human-driven (CI workflow step or external terminal) for v3.7.0.

---

**Goal:** 將 ARS 從「git clone + symlink 到 `~/.codex/skills/`」的分發模式，升級成 Codex Plugin（`/plugin install` 一行裝），同時保留 GitHub repo 作為 source of truth 與教學內容主入口。

**Architecture:** 雙軌並行——同一個 repo 同時支援 (a) 傳統 clone+symlink 安裝、(b) Codex plugin marketplace 安裝。Plugin 化只新增 `.codex-plugin/`、`commands/`、`agents/`、`hooks/` 四個目錄，不動 `deep-research/`、`academic-paper/`、`academic-paper-reviewer/`、`academic-pipeline/` 四個既有 skill 目錄的 SKILL.md 內容。

**Tech Stack:** Codex Plugin Manifest spec、JSON manifest、Markdown frontmatter（commands/agents）、JSON hooks config、既有 SKILL.md（不變）。

**Platform scope（必讀）：**
- ✅ Codex CLI / VS Code 擴充 / JetBrains 擴充：plugin 完整支援
- ❌ Codex web 網頁版 / Codex for Work / model provider API 直呼：**完全不支援 plugin**，這條路繼續靠 GitHub README + 教學內容
...
- 流量主來源：Google 41% / github.com 19% / chatgpt.com + Codex web（LLM 推薦鏈）
...
**驗收：** 在乾淨的 Codex session 跑 `/plugin install`，4 個 skill 出現在 skill 清單，`/skill deep-research` 可正常觸發。
...
```json
{
  "name": "academic-research-skills",
  "version": "3.7.0",
  "description": "Academic Research Skills for Codex: research → write → review → revise → finalize. 4 skills + 10 modes + Material Passport pipeline.",
  "author": {
    "name": "Cheng-I Wu",
...
Run: `python3 -c "import json; json.load(open('.codex-plugin/plugin.json'))"`
...
- **B. 雙路徑**（保守）：`.codex-plugin/plugin.json` 加 `"skillsPaths": ["./", "skills/"]`（如 spec 支援），或在 manifest 列出顯式 skill 路徑

**決策：採 B**。優先保留既有 4.3k clone 用戶不破。Phase 2 再評估是否乾淨遷移到 `skills/`。

- [ ] **Step 3: 查 plugin.json 是否支援自訂 skill 路徑**

Run: `gh search repos --json name,url "topic:codex-code-plugin" | head -20`
Action: 用 codex-code-guide agent 確認 manifest schema 是否有 `skillsPaths` 或等效欄位；若無，Phase 1 採目錄符號連結方案：
...
- [ ] **Step 1: 用 `codex --plugin-dir` 本地測試**

Run: `cd /tmp && codex --plugin-dir ~/Projects/academic-research-skills`
Expected: Codex 啟動後 `/skill list` 看得到 deep-research / academic-paper / academic-paper-reviewer / academic-pipeline
...
```bash
git add .codex-plugin/ skills/ README.md README.zh-TW.md
git commit -m "feat(v3.7.0): add Codex plugin manifest + marketplace metadata

- .codex-plugin/plugin.json: name, version 3.7.0, repo metadata
...
**目標：** 把 10 個 mode 變成 slash command，agent council 變成 plugin agent，把模型路由（balanced model/frontier model，禁 fast model）寫死在 frontmatter。

**驗收：** 使用者在 session 跑 `/ars-full`、`/ars-revision`、`/ars-lit-review` 等 slash 指令直接觸發對應 mode；agent council 派工自動走 balanced model/frontier model 不掉到 fast model。
...
|---|---|---|---|
| full | `/ars-full` | 完整 pipeline | frontier model（架構決策）|
| plan | `/ars-plan` | 只規劃不寫 | balanced model |
| outline | `/ars-outline` | 大綱 | balanced model |
| revision | `/ars-revision` | 改稿 | balanced model |
| revision-coach | `/ars-revision-coach` | 改稿教練 | frontier model |
| abstract | `/ars-abstract` | 摘要 | balanced model |
| lit-review | `/ars-lit-review` | 文獻回顧 | balanced model |
| format-convert | `/ars-format-convert` | 格式轉換 | balanced model |
| citation-check | `/ars-citation-check` | 引用檢查 | balanced model |
| disclosure | `/ars-disclosure` | AI disclosure | balanced model |

依 `feedback_model_floor.md`：**任何情境絕不用 fast model**。
...
routing (frontier model for full/revision-coach, balanced model for everything else).
No fast model per project policy."
```
...
Expected: slash command 觸發、agent dispatch、hook 跑、無 fast model 出現
...
**驗收：** 在另一台機器 `/plugin marketplace add Imbad0202/academic-research-skills` 後開 auto-update，repo push 新 commit 後重啟 Codex 自動拉新版。

### Task 3.1: 確認 marketplace 自動更新行為

**重要事實（codex-code-guide 已查證；2026-05-05 codex review F-004 校正）：**
...
**Files:**
- Modify: `CHANGELOG.md`
- Modify: `.codex-plugin/plugin.json`（每次 release 同步 version）

- [ ] **Step 1: 將 plugin.json `version` 與 git tag 綁死**

每次 `git tag v3.7.x` 前必須先 bump `.codex-plugin/plugin.json` `version` 欄位。

依 `feedback_version_bump_sweep_checklist.md`：major version bump 時要 sweep 7 類地點。新增第 8 類：`.codex-plugin/plugin.json`。
...
- 新增追蹤：GitHub Releases 下載數 + plugin marketplace install 計數（若 platform 提供 API）
...
| Codex CLI 用戶誤裝 plugin | 低 | README 明寫「plugin = Codex only；Codex 用 academic-research-skills-codex」 |
...
- 平台支援限制（Codex web / cowork / API 不支援）→ 文件最上方 Platform scope
- 工程量 → Phase 1 (1-2h) / Phase 2 (1-2d) / Phase 3 (0.5d)
- 與另一個 session 的隔離 → 專章「與另一個 ARS Session 的隔離」

**2. Placeholder 掃描：** Task 1.2 step 3 還是有「需要 codex-code-guide agent 確認 manifest schema」的動作，但已給出 fallback 路徑（symlink），不算 placeholder。所有 commit message 都是完整可貼的字串，無 TBD/TODO。
