...
This repository is the Codex-native distribution of ARS. Ports to other agent platforms (Opencode, Cursor, Continue, Aider, etc.) are accepted as community-maintained contributions. Two structural shapes are acceptable — both keep core ARS content as the source of truth:
...
- **Model-portability note.** ARS prompts in this fork inherit the active Codex session model rather than pinning provider-specific model tiers. The PR must document which providers/models were tested and where downstream-agent behavior diverged from the Codex baseline.
